...
This project provides a Docker container for [KeePassXC](https://keepassxc.org).
...
services:
  keepassxc:
    image: jlesage/keepassxc
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/keepassxc:/config:rw"
```
...
[Releases]: https://github.com/jlesage/docker-keepassxc/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/keepassxc/tags
...
container, it would proxy all HTTP requests for `keepassxc.domain.tld` to
...
server {
	...
	location = /keepassxc {return 301 $scheme://$http_host/keepassxc/;}
	location /keepassxc/ {
		proxy_pass http://docker-keepassxc/;
...
| **Application version** label | Displays the version of KeePassXC integrated into Docker image. |
...
Any desktop notification generated by KeePassXC is forwarded to the
